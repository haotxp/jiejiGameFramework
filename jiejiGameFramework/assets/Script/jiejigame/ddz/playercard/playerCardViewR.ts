...
@ccclass
export default class playerCardViewR extends cc.Component {
    ...
    awake(){
        ...
        for (let index = 0; index < 10; index++) {
            let pai = this.node.getChildByName('outpai_1').getChildByName('pai_' + index)
            pai.active = false
            pai.addComponent(gamePaiItem)
            this.outPai.push(pai)
        }
    ...
    showOutPai(index:number, mian:string,length:number){
        ...
        sc.shePaiMian(mian)
        if (gameDdzPlayerInfoControl.Instance.dizhuDir == DdzPlayerDirType.RIGHT) {
            this.outPai[index].removeAllChildren()
            if (index == (length-1)) {
                this.setDiZhuBiaoQian(index)
            }
        }
        let dir = DdzPlayerDirType.RIGHT
